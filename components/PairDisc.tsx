...
export const PairDisc: React.FC = () => {
  const [capStyle, setCapStyle] = useState<'sphere' | 'crown'>('crown');
    ...
    <div className="w-full py-24 bg-[var(--noir)] text-[var(--blanc-pur)] overflow-hidden border-t border-[rgba(255,255,255,0.1)]">
      <div className="max-w-[1280px] mx-auto px-6 md:px-16">
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          {/* 3D Model Style Selector */}
          <div className="flex items-center gap-2 bg-white/5 p-1 border border-white/10">
            <button
              onClick={() => setCapStyle('crown')}
              className={`px-3 py-1.5 type-micro tracking-widest uppercase text-xs transition-colors cursor-pointer ${
                capStyle === 'crown'
                  ? 'bg-[var(--rouge)] text-white font-bold'
        ...
        <div className="w-full h-[540px] md:h-[620px] border border-[rgba(255,255,255,0.15)] bg-black/80 relative overflow-hidden flex items-center justify-center p-6 shadow-2xl">
          ...
          <div className="absolute top-6 left-6 z-10 pointer-events-none space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="type-micro text-gray-400 font-mono">
                {capStyle === 'crown' ? 'KING MONOLITH FLACON' : 'FLAGSHIP SPHERE FLACON'}
              </span>
            </div>
          ...
          {/* 3D Flacon Viewer with Floating Embers & Scroll Zoom */}
          <FlaconViewer3D
            imageSrc="/oad50.png"
